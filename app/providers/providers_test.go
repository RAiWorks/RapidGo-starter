...
// TC-08: Provider registration order ΓÇö Config before Logger
...
func TestMiddlewareProvider_CSRFConditionalOnMode(t *testing.T) {
	// API mode ΓÇö CSRF should NOT be registered
	middleware.ResetRegistry()
	c := container.New()
	...
	func() {
		defer func() {
			if r := recover(); r == nil {
				t.Fatal("expected panic when resolving 'csrf' in API mode ΓÇö it should not be registered")
			}
		}()
		middleware.Resolve("csrf")
	}()

	// Web mode ΓÇö CSRF SHOULD be registered
	middleware.ResetRegistry()
	c2 := container.New()
	...
	// All mode ΓÇö CSRF SHOULD be registered (all includes web)
	middleware.ResetRegistry()
	c3 := container.New()
